...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    fun onClick(view: View) {
        ...
        if (view.id == R.id.RightClick) {
            if (null != clientThread) {
                clientThread!!.sendMessage("R")
            }
            return
        }
        if (view.id == R.id.LeftClick) {
            if (null != clientThread) {
                clientThread!!.sendMessage("L")
            }
            return
        }
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if(len1 >=4){len1 = 4
            val1 = val1.substring(0,len1)
        }
        else if(len1< 4){val1 = "0.00"
            }
        if(len2 >=4){len2 = 4
            val2 = val2.substring(0,len2)

        }
        else if(len2< 4){val2 = "0.00"
        }
        if(len3 >=4){len3 = 4
            val3 = val3.substring(0,len3)

        }
        else if(len3< 4){val3 = "0.00"
        }
        ...
        Log.d("ACC", val1+"|" + val2+ "|"+ val3 )
        accvalue =  val1+ "," + val2+","+val3
        //Log.d("ACC", accvalue )
        if (null != clientThread) {
            clientThread!!.sendMessage("S"+accvalue+"E")
        }
